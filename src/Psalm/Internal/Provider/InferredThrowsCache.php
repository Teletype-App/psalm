 ...
 *     conditions?: array<lowercase-string, array<string, list<array<int, bool|int|string|null>>>>,
...
final class InferredThrowsCache
{
    ...
    public function __construct(
        ...
        if (is_array($decoded) && ($decoded['schema'] ?? null) === 4
        ...
        foreach ($this->state['entries'] as $file => $entry) {
            ...
            foreach ($entry['method_offsets'] ?? [] as $id => $offset) {
                ...
                $valid['conditions'][$id] = $entry['conditions'][$id] ?? [];
    ...
    public function save(array $entries, array $edges): void
    {
        ...
        foreach ($entries as $file => &$entry) {
            ...
            $entry['conditions'] = ($entry['conditions'] ?? []) + ($previous['conditions'] ?? []);
        ...
        unset($entry);
        $state = ['schema' => 4, 'global' => $this->snapshot['global'],
